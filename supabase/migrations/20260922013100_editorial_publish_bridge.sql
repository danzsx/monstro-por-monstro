-- Allow an administrator publication to update the legacy read model used by the MVP app.
grant select, insert, update, delete on public.topics, public.topic_prerequisites, public.questions to authenticated;
create policy admin_topics_write on public.topics for all to authenticated using (public.is_admin()) with check (public.is_admin());
create policy admin_prerequisites_write on public.topic_prerequisites for all to authenticated using (public.is_admin()) with check (public.is_admin());
create policy admin_questions_write on public.questions for all to authenticated using (public.is_admin()) with check (public.is_admin());
