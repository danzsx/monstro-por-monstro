...
describe('vínculos com monstros e progresso', () => {
  test('vincula os treze monstros de Natureza sem fabricar entradas no catálogo', () => {
    ...
    expect(resolved.filter(n => n.topic).map(n => n.topic!.id).sort()).toEqual(['calorimetry', 'cytology', 'ecology', 'electricity', 'electrochemistry', 'evolution', 'genetics', 'human-physiology', 'kinematics', 'newton-laws', 'ph-hydrolysis', 'solutions', 'stoichiometry']);
