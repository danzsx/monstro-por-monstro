...
export interface BattleState {
  decision: NextMonsterDecision; plan: BattlePlan; phase: 'check-in' | 'barrier' | 'intervention' | 'lesson' | 'question' | 'feedback' | 'complete';
...
export function battleReducer(state: BattleState, action: BattleAction): BattleState {
  switch (action.type) {
    ...
    case 'ANSWER': return state.phase === 'question' && action.attempt.questionId === state.plan.questionIds[state.questionIndex] ? { ...state, phase: 'feedback', attempts: [...state.attempts, action.attempt] } : state;
    case 'NEXT':
      ...
      if (state.phase === 'feedback') return state.questionIndex + 1 < state.plan.questionIds.length ? { ...state, questionIndex: state.questionIndex + 1, phase: 'question' } : { ...state, phase: 'complete' };
      return state;
  }
}
